...
// CSS, Benchpress şablonuna gömülemez (CSS'teki {} ile çakışır). Bu yüzden
// stiller burada bir <style> olarak <head>'e bir kez enjekte edilir.
const DEU_CSS = `
.deu-announcements{
	--deu-navy:#00457c; --deu-navy-d:#003257; --deu-burgundy:#8c1d40; --deu-accent:#0a5fa3;
	--deu-surface:var(--bs-body-bg,#fff); --deu-text:var(--bs-body-color,#1f2937);
	--deu-muted:var(--bs-secondary-color,#6b7280); --deu-line:var(--bs-border-color,rgba(17,24,39,.08));
	--deu-hover:rgba(10,95,163,.06);
	position:relative; border:1px solid var(--deu-line); border-radius:16px; overflow:hidden;
	background:var(--deu-surface); box-shadow:0 1px 2px rgba(17,24,39,.04),0 8px 24px -12px rgba(17,24,39,.12);
...
.deu-icon{flex:0 0 auto; width:42px; height:42px; border-radius:12px; background:rgba(255,255,255,.16);
...
.deu-item{position:relative; display:block; padding:.7rem 1.05rem .7rem 1.15rem;
	border-bottom:1px solid var(--deu-line); transition:background .12s ease; animation:deu-in .4s ease both;}
.deu-item:last-child{border-bottom:0;}
.deu-item::before{content:""; position:absolute; left:0; top:.55rem; bottom:.55rem; width:3px;
	border-radius:0 3px 3px 0; background:var(--deu-dot,var(--deu-accent)); opacity:0; transition:opacity .15s ease;}
...
html[data-bs-theme="dark"] .deu-announcements{--deu-accent:#4f9fe0; --deu-hover:rgba(79,159,224,.1);
	box-shadow:0 1px 2px rgba(0,0,0,.3),0 10px 28px -14px rgba(0,0,0,.5);}
`;

$(document).ready(function () {
	if (!document.getElementById('deu-announcements-css')) {
		const style = document.createElement('style');
		style.id = 'deu-announcements-css';
		style.textContent = DEU_CSS;
